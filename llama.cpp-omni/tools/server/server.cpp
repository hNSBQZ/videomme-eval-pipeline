...
int main(int argc, char ** argv) {
    ...
    const auto handle_stream_prefill_impl = [&ctx_server, &res_ok, &res_error](const json & data, httplib::Response & res) -> void {
        ...
        const std::string prompt             = data.value("prompt", "");
        const bool skip_system_prompt        = data.value("skip_system_prompt", false);
        ...
        std::lock_guard<std::mutex> lock(ctx_server.octx_mutex);
        if (!stream_prefill(ctx_server.octx, aud_fname, img_fname, cnt, max_slice_nums, prompt, skip_system_prompt)) {
            ok = false;
            // break;
        }
        // }
    ...
    const auto handle_stream_reset_impl = [&ctx_server, &res_ok, &res_error](const json & data, httplib::Response & res) -> void {
        ...
        
        {
            ...
            // 重置 system_prompt_initialized，使下次 prefill(index=0) 重新初始化 system prompt
            ctx_server.octx->system_prompt_initialized = false;
            SRV_INF("%s: system_prompt_initialized reset to false\n", __func__);
